...
function getProductTokens(product = {}) {
    ...
    // Build token variants AND expand with synonyms for better matching
    const baseTokens = buildTokenVariants(haystack);
    const expandedTokens = expandTokens(baseTokens);
    
    product.searchTokens = expandedTokens;
...
export function findMatchingProducts(query = '', products = [], limit = null) {
    ...
    const scored = products
        .map((product) => {
                ...
                if (haystackSet.has(queryToken)) {
                    ...
                    for (const haystackToken of haystackTokens) {
                        if (isFuzzyMatch(queryToken, haystackToken)) {
                            // Fuzzy match - lower score than exact
            ...
            return { product, score, exactMatches, fuzzyMatches };
        })
        .filter(({ score }) => score > 0)
        .sort((a, b) => {
            // Sort by score first, then by exact matches
