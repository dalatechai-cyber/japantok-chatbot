...
const TOKEN_SYNONYMS = {
    ...
    harer: ['harrier', 'харриер'],
    harier: ['harrier', 'харриер'],
    bumper: ['бампер', 'гупер'],
    'бампер': ['bumper', 'гупер'],
    'гупер': ['бампер', 'bumper']
};
    ...
    tokens.forEach((token) => {
        ...
        if (synonyms) {
            synonyms.forEach((synonym) => {
                const normalized = normalizeForSearch(synonym);
...
export function findMatchingProducts(query = '', products = [], limit = 6) {
    ...
    const tokens = expandTokens(buildTokenVariants(trimmed));
    ...
    const scored = products
        .map((product) => {
            const haystack = [product.name, product.model, product.tokCode, product.oemCode]
                .filter(Boolean)
                .join(' ');
            const haystackText = ` ${buildTokenVariants(haystack).join(' ')} `;
            ...
            const tokCode = product.tokCode?.toLowerCase();
            const oemCode = product.oemCode?.toLowerCase();

            if (tokCode && tokenText.includes(tokCode)) {
                score += 25;
            }
            if (oemCode && tokenText.includes(oemCode)) {
                score += 15;
            }
