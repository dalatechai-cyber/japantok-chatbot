...
function getProductTokens(product = {}) {
    ...
    // Include title field for compatibility list search
    const haystack = [product.name, product.title, product.model, product.tokCode, product.oemCode]
        .filter(Boolean)
        .join(' ');
...
export function findMatchingProducts(query = '', products = [], limit = null) {
    ...
    const scored = products
        .map((product) => {
            ...
            // Build normalized full product text for substring matching
            // Prioritize title field for compatibility list search
            const fullProductText = normalizeForSearch(
                [product.title, product.name, product.model, product.tokCode, product.oemCode]
                    .filter(Boolean)
                    .join(' ')
            );
            ...
            tokens.forEach((queryToken) => {
                if (haystackSet.has(queryToken)) {
                    ...
                    if (!foundSynonym) {
                        ...
                        // If no fuzzy match, check for substring match within product text (especially title)
...
/**
 * Extract a clean display name from the title field
 * If title is a long compatibility list, truncate it intelligently
 * @param {string} title - The title/compatibility list
 * @param {string} tokCode - The TOK code as fallback
 * @param {number} maxLength - Maximum length before truncation
 * @returns {string} Clean display name
 */
export function extractCleanDisplayName(title = '', tokCode = '', maxLength = 100) {
    if (!title) return tokCode || 'Тодорхойгүй';
    ...
    // Title is too long - likely a compatibility list
    // Extract first few items and add ellipsis
    ...
    const breakPoint = Math.max(lastComma, lastSlash, lastSpace);
    if (breakPoint > maxLength * 0.7) { // If we found a good break point in the last 30%
...
export function formatProductsForPrompt(products = []) {
    ...
    return products
        .map((product, index) => {
            ...
            const displayName = extractCleanDisplayName(product.name, product.tokCode);

            return (
                `${index + 1}. Нэр: ${displayName}` +
...
export function summarizeProductsForClient(products = []) {
    return products.map((product) => ({
        id: product.id,
        name: extractCleanDisplayName(product.name, product.tokCode),
...
export function extractCategories(products = []) {
    ...
    products.forEach((product) => {
        // Use title field for category extraction (full compatibility list/product name)
...
function normalizeProduct(row = {}) {
    ...
    // Prioritize English headers, fallback to Mongolian for backward compatibility
    const title = pickField(cleaned, ['title', 'Title', 'Барааны нэр', 'Нэр', 'name', 'Product Name']);
    const tokCode = pickField(cleaned, ['tok_code', 'tokCode', 'tok', 'TOK код', 'TOK CODE', 'Tok code']);
    const oemCode = pickField(cleaned, ['oem', 'oemCode', 'OEM', 'OEM код']);
    const model = pickField(cleaned, ['model', 'Model', 'Загвар', 'Машин загвар']);
    // ONLY use price_wholesale_with_vat as the display price
    const priceWholesaleWithVat = pickField(cleaned, ['price_wholesale_with_vat', 'priceWholesaleWithVat', 'Бөөний үнэ (НӨАТ орсон үнэ)', 'Үнэ (НӨАТ-тэй)', 'priceVat']);
    const stock = pickField(cleaned, ['stock', 'Stock', 'Нөөц', 'Тоо']);
    const brand = pickField(cleaned, ['brand', 'Brand', 'Брэнд']);

    const priceWithVat = formatPrice(priceWholesaleWithVat);

    return {
        id: pickField(cleaned, ['id', 'ID', 'Row ID']) || tokCode || oemCode || cryptoRandomId(),
        // Note: Both name and title store the full text. Truncation happens only during display
        // in formatProductsForPrompt() and summarizeProductsForClient() using extractCleanDisplayName()
        name: title, // Full product name/compatibility list for internal use
        title: title, // Full title/compatibility list specifically for search operations
        tokCode,
        ...
        model,
        brand,
        priceWithVat, // Only VAT-included wholesale price
        priceWithoutVat: undefined, // Deprecated - not used anymore
        rawPriceWithVat: priceWholesaleWithVat,
        rawPriceWithoutVat: undefined, // Deprecated - not used anymore
        stock,
        raw: cleaned
    };
}
